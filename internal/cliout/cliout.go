...
// Sanitize neutralizes an untrusted, server- or peer-sourced string before
// it is printed to the user's terminal. Backend-relayed values (DM bodies,
// usernames, log content/model names, abuse-report fields, error messages)
// can carry attacker-chosen bytes — left raw, an embedded ESC/CSI/OSC
// sequence would drive the reader's terminal (cursor moves, line hiding,
// title/clipboard writes, display spoofing). It drops C0 control bytes
// (<0x20, except tab), DEL and C1 controls (0x7F-0x9F), and strips ANSI
// escape sequences (CSI, OSC, and other ESC-introduced forms), leaving
// ordinary text — including multi-byte UTF-8 — untouched so the display
// width of normal content is unchanged. Apply any trusted style.* wrapping
// AFTER sanitizing, never before, or the legitimate escapes are stripped too.
