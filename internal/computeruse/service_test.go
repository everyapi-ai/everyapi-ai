...
func TestResolveAppAllowsAllApplicationTypes(t *testing.T) {
	for _, marker := range []string{"", "1"} {
		t.Run("legacy-browser-marker="+marker, func(t *testing.T) {
			t.Setenv("EVERYAPI_COMPUTER_USE_BLOCK_BROWSERS", marker)
			for _, bundleID := range []string{
				"com.apple.Safari", "com.google.Chrome.canary",
				"com.google.Chrome.app.example", "com.openai.atlas",
				"com.apple.Terminal", "COM.APPLE.TERMINAL", "com.googlecode.iterm2",
				"com.1password.1password", "com.bitwarden.desktop",
				"com.apple.keychainaccess", "com.apple.passwords",
				"com.apple.systempreferences", "ai.everyapi.connect", "com.apple.TextEdit",
			} {
				t.Run(bundleID, func(t *testing.T) {
					provider := fixtureProvider()
					want := App{Name: "Fixture", BundleID: bundleID, PID: 91}
					provider.apps = []App{want}
					service := NewService(provider, newMemoryStore(), time.Now)
					for _, selector := range []string{bundleID, want.Name, "pid:91"} {
						got, err := service.ResolveApp(context.Background(), selector)
						if err != nil {
							t.Fatalf("ResolveApp(%q): %v", selector, err)
						}
						if got.BundleID != want.BundleID || got.PID != want.PID {
							t.Fatalf("ResolveApp(%q) = %#v, want %#v", selector, got, want)
						}
					}
				})
			}
