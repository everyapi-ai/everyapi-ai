// Package tools is the registry of third-party CLIs that `relaya use`
// knows how to point at Relaya. Adding a tool here is a single map
...
//     Relaya gateway. URLs are computed at runtime by appending the
//     tool-specific suffix to the user's configured API base, so a
//     local-dev base (http://localhost:3000) works without per-tool
//     env edits.
...
// Tool describes how to launch one third-party CLI against Relaya.
type Tool struct {
	Name        string
	ExecName    string
	InstallHint string
...
// user types (`relaya use <name>`). Names are lowercase.
var Registry = map[string]*Tool{
	...
	// matches Google's published path; Relaya's gemini relay is
...
// the no-arg `relaya use` interactive picker and by Lookup's error
// message.
func Names() []string {
