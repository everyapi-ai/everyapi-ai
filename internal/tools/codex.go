...
func writeCodexOfficialConfigTOMLWithCatalog(codexHome, catalogPath, bootModel string) error {
	...
	body := []byte("# Auto-generated by `everyapi use codex --transparent`.\n" +
		"# Uses the official OpenAI API origin through EveryAPI's local connector.\n" +
		...
		"name = \"OpenAI through EveryAPI\"\n" +
		"base_url = \"https://api.openai.com/v1\"\n" +
		"env_key = \"OPENAI_API_KEY\"\n" +
		"wire_api = \"responses\"\n" +
		"requires_openai_auth = false\n" +
		// EveryAPI currently relays Responses over HTTP. Declaring that boundary
		// prevents Codex from logging a failed WebSocket handshake before its
		// automatic HTTP fallback, while standalone search remains available.
...
func writeCodexConfigTOMLWithCatalog(codexHome, apiBase, catalogPath, bootModel string) error {
	...
	// EveryAPI's gateway currently serves Responses over HTTP. Be explicit so
	// a future Codex default cannot add a noisy failed WebSocket handshake.
