...
// TestUnixExitCode covers the three classifications: clean exit, a normal non-zero exit code, and termination by signal (128+signo). unixExitCode is what the Unix Exec actually propagates; it layers the signal mapping over the shared exitCodeFromWait.
func TestUnixExitCode(t *testing.T) {
	...
	// The shared base mapping (no signal layer): signaled -> ExitCode() which is -1, normal exit -> its code, nil -> 0.
...
// TestExecPropagatesChildExitCode runs the test binary as a subprocess that calls Exec with a fake "tool" (`sh -c 'exit 7'`). Exec launches it as a child, waits, and os.Exits with its code — so the subprocess must exit 7. This is the standard os/exec helper-process pattern; it exercises the real Start/Wait/exit-code path without a TTY and without killing the outer test process (Exec's os.Exit only fires in the child). The helper branch is gated by an env var so it's inert under normal `go test`.
func TestExecPropagatesChildExitCode(t *testing.T) {
	...
	// Redirect the helper's config dir into a temp dir: Exec now writes a use.log launch/exit line, and without this the helper would append to the developer's real ~/.config/everyapi/use.log.
...
func TestExecWithOptionsRunsCleanupBeforeExit(t *testing.T) {
	...
	// Redirect the helper's config dir so its use.log write lands in the temp dir, not the developer's real ~/.config/everyapi.
	cmd.Env = append(os.Environ(),
