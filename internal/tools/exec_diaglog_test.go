...
// TestUnixExitCause renders the diagnostic cause string for each fate a supervised child can meet. The signal case is the one that matters for the "all sessions died silently at once" investigation: a batch of children reaped by the same signal (SIGKILL from OOM, SIGHUP from an ssh hangup) is only distinguishable after the fact from this text.
func TestUnixExitCause(t *testing.T) {
	...
	// SIGKILL(9) is the OOM killer's fingerprint — the exact case the log exists to make legible.
...
// TestSignalCauseNamesNumber verifies the parent-received-signal line carries the signal NUMBER, matching the "signal N" shape of the child exit line so an operator can grep one number across both.
