...
// sellerNetTimeout bounds a single upstream OAuth HTTP exchange (eligibility, /start, /complete) so a hung or slow backend can't wedge the CLI indefinitely even when the user isn't watching to press Ctrl+C. The interactive waits — device-code poll, loopback callback, stdin paste — are deliberately NOT bounded by this; they have their own, much longer, user-facing budgets and are SIGINT-cancellable via the parent signalCtx.
const sellerNetTimeout = 60 * time.Second

// sellerExchangeCtx derives a bounded child of parent for one discrete network round-trip. Caller MUST call the returned cancel right after the call returns (not deferred — these handlers make several in a row and we want each timer released promptly).
...
// sellerAddOAuth is the seller OAuth-based onboarding bridge: the CLI walks the seller through the provider's OAuth flow and the backend mounts the resulting credential as their channel — the seller NEVER copies a token string by hand. docs/cli/channel-marketplace.md §7-1 calls this the "game changer" for seller onboarding.
//
...
//   - antigravity — loopback OAuth (local listener); the ONLY feasible path since antigravity's Google client accepts no hosted/device redirect, which is why it can't be connected from the web dashboard
//
// Usage:
//
...
func sellerAddOAuthCodex(args []string) error {
	...
	*models = strings.TrimSpace(*models)
	...
	// WithCookieJar is the load-bearing call here: backend stashes the device_auth_id / user_code / name / models keyed by a session cookie set on /start. Without a jar the /poll call would land in a fresh session and the handler would 200 with code=expired.
	client := api.ForCredentials(creds).WithCookieJar()
	ctx, stop := cliout.SignalCtx()
	defer stop()

	// Pre-check eligibility — same reasoning as `add-key`: failing a gate AFTER the seller authorised in their browser is a worse UX than telling them now to fix it first.
	...
	if !*noBrowser {
		// Same fail-soft approach as `everyapi auth login`: a missing xdg-open / `open` shouldn't crash; the URL is already printed for the user to copy.
	...
	if err != nil {
		...
		case errors.Is(err, context.Canceled):
			// Ctrl+C during the poll cancels the context; exit cleanly instead of dumping "Error: poll: context canceled" with a non-zero status, matching `everyapi auth login`.
...
// sellerAddOAuthClaude drives the Anthropic Claude OAuth flow. Unlike codex device flow, Claude can't be auto-completed by the CLI: Anthropic's OAuth provider hard-codes `redirect_uri` to console.anthropic.com/oauth/code/callback, so we can't open a local listener to receive the code. The user has to paste a short string from the browser back into the terminal — annoying but still much better than chasing ~/.claude/auth.json yourself.
//
// Usage:
//
...
func sellerAddOAuthClaude(args []string) error {
	...
	*models = strings.TrimSpace(*models)
	...
	// WithCookieJar same as codex: backend stashes state/verifier/ name/models under a session keyed by `everyapi_session` cookie, and the complete call has to land in the SAME session.
	client := api.ForCredentials(creds).WithCookieJar()
	ctx, stop := cliout.SignalCtx()
	defer stop()

	// Eligibility pre-check — fail before we burn a round-trip to Anthropic's authorize endpoint.
	ecx, ecancel := sellerExchangeCtx(ctx)
	elig, err := client.GetSellerEligibility(ecx)
	ecancel()
	if err != nil {
...
// sellerAddOAuthGemini is the true one-click OAuth path. Google's gemini-cli installed-app client accepts http://127.0.0.1:<port>/ callback as a redirect_uri, so the CLI runs its own listener and Google sends the code straight to it — no manual paste like claude, no user_code typing like codex.
//
// Usage:
//
...
//  3. Tell backend `redirect_uri = http://127.0.0.1:<port>/callback`, receive the Google authorize URL + state (backend validates redirect is a real loopback before handing it back)
//  4. Open browser
//  5. Block on listener until Google redirects to /callback with `?code=…&state=…` OR `?error=…`
//  6. Forward code+state to backend's /complete, which exchanges with Google and mints the channel
func sellerAddOAuthGemini(args []string) error {
	...
	*models = strings.TrimSpace(*models)
	...
	// Start the listener BEFORE eligibility check — we need the loopback URL to hand to /start. Eligibility-fail path Close()s the listener immediately so the port is held for milliseconds.
	...
	// State check: the callback MUST carry the same state we got back from /start. Otherwise this is either a stale flow (browser re-played an old URL) or a forged hit. Bail with a clear message — the backend also checks but failing locally avoids a wasted round-trip.
...
// sellerAddOAuthAntigravity drives Google Antigravity's OAuth. It is the SAME loopback flow as gemini — antigravity's Google client only accepts http://127.0.0.1:<port>/callback redirects (no hosted-paste page, no device flow), which is exactly why this can't be done from the web dashboard and must run from a local listener here. The only wire difference from gemini is the backend endpoint, which authorizes the Antigravity desktop client (whose Code Assist tier serves the antigravity model set).
//
// Usage:
//
...
func sellerAddOAuthAntigravity(args []string) error {
	...
	*models = strings.TrimSpace(*models)
	...
	// Start the listener BEFORE eligibility — we need the loopback URL to hand to /start (same ordering as gemini).
	listener, err := oauthloopback.Listen()
	if err != nil {
		return fmt.Errorf("loopback listen: %w", err)
	}
