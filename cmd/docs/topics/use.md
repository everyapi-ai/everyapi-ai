...
Computer Use has a separate, target-scoped safety split. When your task explicitly involves a local macOS app, the agent is told to proactively use `everyapi computer` rather than wait for you to name the command. The launch sets a process-scoped browser fence, so browser targets are disabled by default for `everyapi use` agents; this marker is a default policy, not an unforgeable isolation boundary. Direct `everyapi computer` invocations can operate browsers when the user explicitly requests it. The agent may inspect capabilities and permissions, resolve the in-scope app, list that app's windows, read its accessibility state, and take a window-scoped screenshot when pixels are necessary. It must not inspect unrelated apps or request a permission grant on its own.
