...
func TestClientAddArgv(t *testing.T) {
	...
	// regressions here mean `relaya mcp install <client>` silently
	// registers the wrong thing.
	cases := []struct {
		client string
		want   []string
	}{
		{"claude", []string{"mcp", "add", "relaya", "relaya", "mcp"}},
		{"codex", []string{"mcp", "add", "relaya", "--", "relaya", "mcp"}},
		{"gemini", []string{"mcp", "add", "relaya", "relaya", "mcp"}},
	}
	for _, tc := range cases {
		t.Run(tc.client, func(t *testing.T) {
			...
			got := c.AddArgv("relaya", "relaya", []string{"mcp"})
...
func TestClientRemoveArgv(t *testing.T) {
	...
	for _, name := range []string{"claude", "codex", "gemini"} {
		...
		got := c.RemoveArgv("relaya")
		want := []string{"mcp", "remove", "relaya"}
...
func TestClientManualSnippet(t *testing.T) {
	...
	for _, name := range []string{"claude", "gemini"} {
		t.Run(name+"/parses-as-json", func(t *testing.T) {
			...
			r, ok := got.McpServers["relaya"]
			if !ok {
				t.Fatalf("ManualSnippet missing mcpServers.relaya:\n%s", c.ManualSnippet)
			}
			if r.Command != "relaya" || !reflect.DeepEqual(r.Args, []string{"mcp"}) {
				t.Errorf("ManualSnippet relaya entry = {%q, %v}, want {\"relaya\", [\"mcp\"]}", r.Command, r.Args)
			}
		})
	}
	...
	t.Run("codex/structure", func(t *testing.T) {
		...
		for _, frag := range []string{"[mcp_servers.relaya]", `command = "relaya"`, `args = ["mcp"]`} {
