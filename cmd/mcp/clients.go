...
type mcpClient struct {
	...
	// command is `serverCmd` with `serverArgs`. The relaya install
	// path always calls this with serverName="relaya",
	// serverCmd="relaya", serverArgs=[]string{"mcp"}.
...
// (`relaya mcp install <client>`). Names are lowercase. Order in the
...
var mcpClients = map[string]*mcpClient{
  ...
  "mcpServers": {
	...
	"codex": {
		...
		ManualSnippet: `[mcp_servers.relaya]
command = "relaya"
args = ["mcp"]`,
		AddArgv: func(name, cmd string, args []string) []string {
