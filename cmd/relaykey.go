...
// resolveRelayKey returns a relay API key (sk-everyapi-…) for the relay
...
// exactly why `everyapi use` 401'd. So:
//
...
// enabled key, full stop. A `--token=<name>` flag on `everyapi use`
...
// (from `everyapi use --group`/`--channel`) deliberately routes to the
