...
func TestResolveToolModel(t *testing.T) {
	...
	t.Run("promotional metadata keeps dedicated chat models selectable", func(t *testing.T) {
		t.Setenv(env, "")
		...
		if got := os.Getenv(env); got != "gpt-5.6-luna" {
			t.Errorf("%s = %q, want the first promotional dedicated chat model", env, got)
		}
	})
