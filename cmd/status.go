...
// styledQuota renders the "remaining / used" body of the quota line. The amounts are marked with **…** in status.remaining_used across every locale; routing the formatted string through style.Emph bolds them on a styled terminal and strips the markers to plain text when output is piped / NO_COLOR — keeping `everyapi auth status | grep` parseable. Markers live in the format string, never in the data, so formatting must happen before the Emph pass.
...
// Status renders the user's quota and usage in USD. Reads quota_per_unit from /api/status (unauthenticated) so a stale token produces a clean 401 from /api/user/self rather than a confusing "got JSON but no quota_per_unit" path.
//
// SIDE EFFECT: on first run after upgrade (when credentials.json has no relay_key), resolveRelayKey resolves and caches the relay API key, rewriting credentials.json. Subsequent runs are read-only. We accept the asymmetry because the alternative — making the user run `everyapi auth login` after upgrade — is worse UX, and the rewrite is a one-time migration.
func Status(args []string) error {
	...
	// OAuth2 relay-key logins (the fallback when the legacy /api/cli/device-auth-start is absent) carry no management session: the stored access token IS the relay key and there's no user id, so GetSelf/quota can't work. Don't mislead the user with the "session expired — re-login" string below (re-login via the same OAuth2 path reproduces this exact state). Show relay health only.
	...
	if err != nil {
		// GetStatus (public) just succeeded, so the backend is reachable — a rejection of the authenticated self call means the session is bad. That arrives either as a 401 or, for an invalid token, as the backend's HTTP-200 envelope rejection (EnvelopeError); both map to the friendly "session expired, re-login" message.
	...
	// Lazy-migrate the cached role for credentials.json files written before the Role field existed. Old creds end up with Role=0 (treated as non-admin → help hides admin block); the first `everyapi auth status` from an admin user repopulates it without needing them to re-login. Save errors are non-fatal — status display is the primary job here.
	...
	if perUnit <= 0 {
		// Defensive: server should always send a positive value; if it doesn't, fall back to displaying raw quota integers rather than dividing by zero.
		perUnit = 1
	}
...
func printRelayHealth(ctx context.Context, creds *config.Credentials) {
	...
	case rkErr != nil:
		// Token lookup itself failed (transport, 5xx, etc.). Not a verdict on the key — just say we couldn't check.
