...
2. **`everyapi use <tool>` 把 token 通过 env 传给子进程** — claude / codex / gemini 的 debug 日志可能落档 env。**对策**：分享 debug log 前 redact `*_TOKEN` / `*_API_KEY`。
3. **MCP server 凭据继承** — `everyapi mcp` 读同一份 credentials 文件，自动跑所有 tool。任何能 spawn `everyapi mcp` 的本机进程（恶意 MCP host、被入侵的 AI agent）都拥有相同权限。**对策**：只在信任的 MCP host 里配 `everyapi`；钱路工具 `everyapi_seller_withdraw` 已加 `confirm: "yes"` 必填字段做 friction step。
4. **Release 二进制无 OS 级 code signing** — macOS Gatekeeper / Windows SmartScreen 会拦。**对策**：每个 release 附带 `SHA256SUMS` + sigstore cosign keyless 签名（`SHA256SUMS.sig` + `SHA256SUMS.pem`），证明 `SHA256SUMS` 是由 `everyapi-ai/everyapi` 仓的 `cli-release.yml` workflow 在 GitHub Actions 上产生的。务必按 README §安装 部分给的命令走完两层校验；不要绕过 OS 警告直接运行未校验的二进制。OS notarization 路线图中。
