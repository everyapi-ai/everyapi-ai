...
| `everyapi artifacts <sub>` | Publish and manage self-contained HTML reports (`share` / `list` / `update` / `delete`) |
...
On macOS, `everyapi computer` drives a small, independently code-signed helper app (`EveryAPI Computer Use.app`, built from `clients/desktop/native/computer-use-macos`) over a local Unix socket, downloading and launching it automatically on first use if it is not already installed — including when EveryAPI Connect has already installed its own bundled copy, which this CLI reuses instead of downloading a second one.
...
everyapi computer click --app com.apple.TextEdit --window-id 42 --element-index 12 --mouse-button right --click-count 2 --modifiers cmd
printf '%s' 'safe text' | everyapi computer set-value --app com.apple.TextEdit --window-index 0 --element-index 12 --value-stdin
printf '%s' 'pasted text' | everyapi computer paste-text --app com.apple.TextEdit --window-index 0 --element-index 12 --text-stdin
everyapi computer hotkey --app com.apple.TextEdit --window-index 0 --key cmd+a
everyapi computer screenshot --app com.apple.TextEdit --window-index 0 --out ./window.png
```

Run `everyapi computer permissions --request accessibility` and `everyapi computer permissions --request screen-recording` to ask macOS to register the separately signed **EveryAPI Computer Use** helper and present its consent UI. The helper cannot approve itself; confirm the system prompt or enable it under System Settings > Privacy & Security, then run `everyapi computer permissions --json` to recheck. If a previously dismissed prompt does not reappear, add the helper manually with the **+** button (`~/Library/Application Support/everyapi/computer-use/EveryAPI Computer Use.app` if the CLI downloaded its own copy, or `/Applications/EveryAPI Connect.app/Contents/Resources/EveryAPI Computer Use.app` if it is reusing Connect's bundled copy). Because the helper is its own signed app with its own bundle identity, that grant is scoped to this one capability: it does not also authorize every AppleScript or JXA script on the machine, and it survives CLI and helper updates. `permissions` reports Accessibility directly; Automation as `unknown`, since this provider does not depend on System Events; and Screen Recording via `CGPreflightScreenCaptureAccess`.

Element indexes come from the latest `get-app-state` snapshot and expire after two minutes. Windows are selected by index (`--window-index`) or by the id `list-windows`/`get-app-state` report (`--window-id`, mutually exclusive with `--window-index`) — either way identified internally by the real per-window id CoreGraphics assigns on screen where one is available, falling back to a snapshot-scoped synthetic id for minimized windows; the provider also uses an internal fingerprint to detect observable changes, but public Accessibility attributes cannot prove that a replacement window or control with identical attributes is the same native instance. The cache stores only opaque application, process, window, path, role, frame, action-name, and fingerprint data under `~/.config/everyapi/computer-use/state/` with private permissions. Obtain a fresh snapshot after `app_stale`, `element_stale`, or `window_stale`. A successful GUI action remains successful when its best-effort state refresh fails; JSON then includes `refreshError` instead of returning a retryable action error. If the helper call is interrupted or returns an invalid receipt after an action is handed off, `action_outcome_unknown` means the action may already have occurred; refresh state before deciding whether to retry.

Every command accepts `--session <id>` (letters, digits, `-`, `_`, `.` only) to namespace that element-index cache. Without it, all callers targeting the same app/window share one cache slot, so two concurrent workflows observing and acting on the same window can overwrite each other's element indexes — a `get-app-state` from one silently invalidates the other's `element-index` references, surfacing as `element_stale` at best or a mismatched click at worst. Give each concurrent workflow its own `--session` value and its cached elements stay isolated from every other session's.

`click` accepts `--mouse-button left|right|middle` (default `left`), `--click-count` (default 1, e.g. `2` for a double-click), and `--modifiers` (a `+`-joined set of `cmd`, `shift`, `alt`/`option`, `ctrl`, matching `hotkey`'s modifier syntax). Any of those beyond the plain-left-single-click default forces a synthesized `CGEvent` click instead of the semantic `AXPress`/`AXConfirm`/`AXOpen` shortcut, since those Accessibility actions have no right-click or multi-click equivalent. `paste-text` delivers text through the pasteboard (save, write, `cmd+v`, restore) instead of synthesizing keystrokes, for element types that reject direct character input; like `type-text`, it rejects text matching the built-in secret detectors. Every action that requires a focused window accepts `--restore-window`, which makes a failed focus check a no-op instead of an error — useful when the target window is deliberately not frontmost. `screenshot` captures the target window only (never the full screen or other windows) via `CGWindowListCreateImage` scoped to that window's id, and requires the Screen Recording grant described above; write the PNG with `--out <path>` or get it base64-encoded in `--json` output. Accessibility-only surfaces such as Finder's desktop and some minimized windows remain readable and actionable, but return `screenshot_unavailable` because they have no real window id and a screen-region fallback could include unrelated pixels.

`get-app-state` and every mutating action also attach a screenshot of the target window to their result by default, taken after the operation completes — pass `--no-screenshot` to skip it. This is a separate, best-effort capture on top of the accessibility snapshot: it never fails the action it is attached to, so a missing Screen Recording grant surfaces as `screenshotError` in JSON output (or a `screenshot unavailable: ...` line in plain output) rather than blocking the click, type, or state read that requested it. On success, the JSON `screenshot` field (or the plain-output `screenshot: <path> (WxH)` line) points at a PNG written under the OS temp directory, not inlined as base64 — copy it out promptly, since files older than an hour are swept the next time a screenshot is taken.

A maintained list of known web browsers, terminal apps, password managers, Keychain Access, Passwords, System Settings, and EveryAPI Connect is blocked as defense-in-depth friction. Browsers are refused for observation as well as for action: an accessibility snapshot of a browser window returns page text from whatever authenticated session happens to be open, so reading one is not meaningfully safer than clicking in it. Matching covers release channels and browser-installed web apps by prefix (`com.google.Chrome.canary`, `com.google.Chrome.app.<id>`, `com.microsoft.edgemac.Beta`), except where a vendor's prefix would overreach — `com.operasoftware.Opera` also matches Opera Mail, so Opera is enumerated per channel instead. Bundle-ID blocking is still not a comprehensive application classifier: unlisted apps, editors with integrated terminals, embedded web views, and renamed or newly released browsers may expose equivalent capabilities; run `everyapi computer list-apps --json` to read a real bundle identifier before adding one. The explicit `--app` target, macOS TCC, and the caller's same-user authority remain the real trust boundary. Observed text is stripped of terminal control sequences and scanned for credentials before output; typed or set text that matches the built-in secret detectors is rejected. Prefer `--text-stdin` and `--value-stdin` to keep ordinary text out of shell history.

`--click-count` must be between 1 and 100. This per-request resource limit bounds native event allocation; larger sequences must be split into separate, freshly targeted actions.

`computer capabilities` includes `independentPointer`: `true` means the running helper can resolve the private macOS window-routing dependency; `false` means it cannot; `null` means no running helper confirmed support (including older helpers or a failed query). This read-only check does not install or launch a helper. Availability does not guarantee that every application or custom control accepts directed events, nor does it replace permission and target-state checks.

Mouse actions use a separate, mouse-transparent indicator labelled **AI**. Coordinate clicks, drags, and scrolls are directed to the selected process/window instead of the global HID stream; the helper does not move, hide, restore, or suppress the physical pointer, and these actions no longer activate the target app first. Direct Accessibility actions remain preferred for ordinary element clicks. Keyboard and paste actions retain their existing focus behavior. The indicator is transient and is not part of the target window's own screenshot pixels.

The coordinate path requires a real on-screen window ID and dynamically resolves the private macOS `CGEventSetWindowLocation` function. If the window disappears or that function is unavailable, the helper returns an error without falling back to global mouse injection. macOS updates may affect compatibility. A successful delivery receipt does not prove that every custom control accepted the event: refresh the target state to verify the requested outcome. Existing application restrictions and permission requirements still apply.
...
**Artifact completion reports**: Claude Code, Codex, OpenCode, and Kilo launches receive a process-scoped delivery standard through their documented instruction surfaces. After completing a task, the agent publishes a sanitized, self-contained HTML report with Summary, Deliverables, Verification, and Follow-ups through `everyapi artifacts share`, then returns the official artifact URL. The CLI tags each upload with the current Git origin repository name (falling back to the workspace directory), so the Dashboard can group reports by project without exposing a local filesystem path. The launcher exposes its exact executable path as `EVERYAPI_CLI_PATH`, so the child uses the same authenticated EveryAPI installation even when `everyapi` is not on `PATH`. An existing report is revised or revoked with `artifacts update` / `artifacts delete`; a publish failure never replaces the normal text result or produces an invented link. The standard never edits project-owned instruction files and is not injected into clients that do not expose a documented process-scoped instruction surface.
...
It is also wired into what `everyapi use` tells the agents it launches. The injected text is a capability list, not just a pointer: an agent that knows `auth status`, `stats usage`, `models list`, and `docs search` exist runs them, and one that does not asks the user to go look it up or answers from training data this platform is not in. The read-only account/platform set is enumerated and runnable unprompted; everything outside it needs an explicit yes naming the command, because `token revoke` / `wallet topup` / `seller withdraw` / `edge remove` move money and destroy access, and `token key` prints a credential while changing nothing.

The same process-scoped instructions advertise local macOS Computer Use without exposing it over MCP. When the user's request explicitly involves a desktop app, an agent proactively uses `everyapi computer` observations against only that target. GUI mutation is allowed only when the request already names a concrete UI outcome; otherwise the agent must ask first. Permission requests are always treated as mutations, and `action_outcome_unknown` requires a fresh observation before any retry.
