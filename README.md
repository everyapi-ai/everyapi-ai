...
**Artifact completion reports**: Claude Code, Codex, OpenCode, and Kilo launches receive a process-scoped delivery standard through their documented instruction surfaces. After completing a task, the agent publishes a sanitized, self-contained HTML report with Summary, Deliverables, Verification, and Follow-ups through `everyapi artifacts share`, then returns the official artifact URL. The launcher exposes its exact executable path as `EVERYAPI_CLI_PATH`, so the child uses the same authenticated EveryAPI installation even when `everyapi` is not on `PATH`. An existing report is revised or revoked with `artifacts update` / `artifacts delete`; a publish failure never replaces the normal text result or produces an invented link. The standard never edits project-owned instruction files and is not injected into clients that do not expose a documented process-scoped instruction surface.
