...
Computer Use accepts any application type, including browsers, terminals, password managers, System Settings, and EveryAPI Connect. Direct CLI calls and `everyapi use` launches use the same application resolution without a bundle-ID blocklist. The explicit `--app` target, macOS TCC, and the caller's same-user authority remain the trust boundary. Observed text is stripped of terminal control sequences and scanned for credentials before output; typed or set text that matches the built-in secret detectors is rejected. Prefer `--text-stdin` and `--value-stdin` to keep ordinary text out of shell history.
