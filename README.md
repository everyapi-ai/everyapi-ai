...
Run `everyapi computer permissions --json` and grant Accessibility to **EveryAPI Computer Use** — not to `everyapi`, `osascript`, or your terminal — under System Settings > Privacy & Security > Accessibility. The helper will not appear there on its own: it only ever queries `AXIsProcessTrusted()` and never calls the prompting variant of that API, so it never registers itself or shows a system permission dialog, consistent with SECURITY.md's guarantee that this feature never opens or modifies permission settings on its own — click the **+** button under Accessibility yourself and browse to the helper app to add it (`~/Library/Application Support/everyapi/computer-use/EveryAPI Computer Use.app` if the CLI downloaded its own copy, or `/Applications/EveryAPI Connect.app/Contents/Resources/EveryAPI Computer Use.app` if it is reusing Connect's bundled copy — `everyapi computer permissions --json`'s error output does not currently say which). Because the helper is its own signed app with its own bundle identity, that grant is scoped to this one capability: it does not also authorize every AppleScript or JXA script on the machine, and it survives CLI and helper updates. `permissions` reports Accessibility directly; Automation as `unknown`, since this provider does not depend on System Events and has no separate Automation preflight to run; and Screen Recording (needed only for `screenshot`) via the non-prompting `CGPreflightScreenCaptureAccess` check, granted the same way under System Settings > Privacy & Security > Screen Recording.
